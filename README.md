# webgl-school
webglの勉強はここに全部いれる

ローカルサーバーはこちらで起動
```
python3 -m http.server
python -m SimpleHTTPServer 8000
```

## 0421やったこと
webglは
openglファミリーに属する
opengl es 2.0

chormeはなかみでアングルが入っているので、見た目が変わる
openglをdiretXに変換しているから
結界GPU、ブラウザ、OSによって見た目が変わるのは当然
https://en.wikipedia.org/wiki/ANGLE_(software)

chormeはGPUのブラックリストをもっている
古いGPUはサポート外 intel HD 3000とか
webglはjsのAPIなので、いくらすごいGPUでも、APIに対応したブラウザでしか動かんよ

iOS８以上ならうごく
Androidは５系以降なら動く

webglの対応状態など
https://webglstats.com/

webgl1.0は2011年
webgl2.0はopengl es 3.0をベースにしている

threejs
https://threejs.org/

デメリット
使ったものが似通う

threejsの隠蔽している部分を少しづつ解説していくスタイル

アンビエントライトは環境光
かなり重い

MeshPhongMaterial
Phongさんが考えた照明
反射光を再現できるモデリングはフォンシェーディングという
反射光は視線を考慮していいる
specularが反射の色

```
const CAMERA_PARAM = {
    // ズームインズームアウト
    fovy: 60,               // field of view Y の略
    aspect: width / height, // カメラが撮影する空間のアスペクト比
    near: 0.1,              // カメラからニアクリップ面までの距離
    far: 10.0,              // カメラからファークリップ面までの距離
    x: 0.0,                 // カメラの X 座標
    y: 2.0,                 // カメラの Y 座標
    z: 5.0,                 // カメラの Z 座標
    lookAt: new THREE.Vector3(0.0, 0.0, 0.0) // カメラの注視する座標
};
```

第一回課題　作品作成

## 0512
やったこと
three.js の基本機能を使いこなせるようになることがメインテーマ

回転の概念
時計の回転をオブジェクトのみだと表現できない
アンカーポイント真ん中で固定される
回転が先に起こるか移動が先に起こるかの問題がある
threejsの世界では必ず先に回転が起きる
生webglなら自由に設定できる

つまり、three.js の内部では、移動と回転を行う処理を実行すると以下のような流れで処理が行われています。

・Object3D に属するオブジェクトを世界に ひとつだけ 配置する
・原点を中心に世界全体を回転させる（常に回転が先）
・オブジェクトを移動させる（回転したあと移動）
・次のオブジェクトを世界に ひとつだけ 配置する
・原点を中心に世界全体を回転させる……（以下繰り返し）

group化することで中身のオブジェクを移動してgroupを回すと時計ができる

・回転という処理の原則
・回転とは、世界全体を回転させること
・つまり言い換えると 回転は常に世界の中心（原点）で行われる
・移動してから回転するのと、回転してから移動するのは意味がまったく違う
・3D では一般に、回転を先に行うことが多い
・three.js でも、やはり回転が先に処理される

透視投影と平行投影
フォグ

霧
クリアした背景が見えてしまう場合フォグの色とリセットしたときの色は同じにしないとうまくうまくいかない
sceneにfogプロパティがある
フォグが適用されるのはあくまでも 描画されるオブジェクトに対してのみ

テクスチャ
JPEG や PNG などの画像が用いられる場合が多い
動画データや Canvas2Dからビットマップデータを作ってテクスチャにできる
ローダークラスを使う（非同期）
load('画像のパス', コールバック関数)


パーティクル
素体ジオメトリを作成
let geometry = new THREE.Geometry();
sizeAttenuation: true // 遠近感を出すかどうかの真偽値 @@@

頂点をまとめる
hree.js はオブジェクトをひとつずつ世界に配置して動かしているので毎度addするとかなり重い

マテリアル設定
depthWrite
手前にあるものの後ろ側が隠れる現実世界では当たり前に起きることを再現する
絵の具は黒くなる、デジタルは色を足すごとに白くなる

## 0526
第三回勉強会の内容
基礎的な数学を身に着けよう

数学は反復
触れる数がおおいと理解できる

ラジアン
日本は度数法がメジャー
プログラムは弧度法（ラジアン）
円周の公式
半径 r の円の円周は……
円周 = 2 * PI * r

３６０ ＝ 2π

ベクトル
 向きと大きさを持った量
 意味わからん
ベクトルは複数の要素が組み合わさって定義される
XY なら二次元ベクトル、XYZ で三次元ベクトル
ベクトルには向きがあり……
向きが同じでも大きさが違うことがある
これがベクトルが向きと大きさを持つ量、と呼ばれる理由


単位ベクトル
長さが１のベクトル
向きだけに注目したい場合
単位化 === 正規化という


ベクトル（3, 5）の長さは？
ベクトル（-4, 2）を単位化すると？
ベクトル（3, 5, 8）の長さは？
ではベクトル（3, 5, 8）のを単位化すると？

Math.sqrt(3*3 + 5*5)
Math.sqrt(-4*--4 + 2*2)
Math.sqrt(-4*-4 + 2*2)
cosnt len = Math.sqrt(-4*-4 + 2*2)
console.log(-4 / len, 2 / len)
console.log(-4 / Math.sqrt(-4*-4 + 2*2), 2 / Math.sqrt(-4*-4 + 2*2))
Math.sqrt(3*3 + 5*5 + 8 * 8)

平行移動とは各軸に平行に動かすこと
threejsはメートル単位

クォータニオン
加算減算はある掛け算は
内積と外積がある
割り算に相当するものはベクトルにはありません
ベクトル同士のなす角

内積や外積はベクトルの積を表すふたつの形
内積は、ふたつのベクトルのなす角を求めることなどに使う
外積は、ふたつのベクトルに直行するベクトルを求めることなどに使う
内積は dot 積とも呼ばれ、数式では文字通り・（ドット）で表す
外積は cross 積とも呼ばれ、同文字通り ✕（クロス）で表す

クォータニオンは軸がない
acosはコサインをラジアンに戻す

ポストプロセス
めっちゃ簡単にできる

## 0609
レンダリングパイプライン
OpenGL を直接叩くのはやめなさい
固定機能パイプラインはない
プログラマブルなパイプラインを使う
シェーダーの出番
固定機能パイプラインを使う人はあまりいないらしい（機能固定により同じような絵作りになる）
WebGLは頂点以外描画できない
頂点とは点
頂点属性を持っている
htmlでいうidとかclass
頂点ひとつひとつ、それぞれに違った情報を持たせる
頂点情報をいれるバッファ　VBO
VBO が出てきたら頂点属性が関係しているな！ と考えて OK
バインドとはCPUからGPUに橋渡しする
ArrayBuffer クラスを使用すると、バイナリデータを取り扱う事ができます。
argumentsは引数を見れる
フラグメントシェーダー１ピクセルずつレンダリングするすごいやつ
シェーダを記述するための専用言語こそが GLSL
シェーダオブジェクトを紐付けてワンセットにするプログラムオブジェクトが必要
シェーダーの紐づけ
ストレージ修飾子
attribute 頂点属性
uniform   バッファのバインドなどをせずに、直接 JavaScript から値を送ることができる唯一の手段です。
varying   vertexからfragmentへの橋渡し　

```
attribute → 頂点属性、つまり VBO の中身
uniform → 直接 JavaScript から値が送れる
varying → 頂点シェーダとフラグメントシェーダをつなぐ
```
頂点シェーダーは頂点の個数分実行される
フラグメントシェーダーは頂点シェーダーによって頂点によって塗りつぶされる
ピクセルで実行される
フラグメントシェーダーのほうが重い

uniform のタイプ指定
4fv

```
4 =　vec4（ストライド）
f = float（浮動小数点）
v = vec(配列)

GLSL で int → 1i
GLSL で float → 1f
GLSL で float の配列 → 1fv
GLSL で vec2 ~ vec4 → 2fv ~ 4fv
GLSL で mat2 ~ mat4 → matrix2fv ~ matrix4fv
```
// それぞれのプリミティブタイプの違いについて理解を深めておきましょう。
// * gl.POINTS //1pxの点
// * gl.LINES　//２つの頂点を１セットとして一本の線ｗｐ引く
// * gl.LINE_STRIP　//頂点をつないでいく
// * gl.TRIANGLES　//３つで1セット　三角形をつくる
// * gl.TRIANGLE_STRIP // 頂点をつないでいきポリゴンを描く

Index Buffer Object（IBO）
課題　五芒星

glTFはjson形式
1.0は汎用性がなかった
用語：Draco
Draco はGoogle が開発した、オープンライセンスの3D モデル専用の超強力な圧縮方式です。3D モデルのデー
タ容量を1/10 程度にまで圧縮することが可能です。
Facebookに対応してる（すごい　glTF-Binary形式のみ）
テクスチャー画像は2 のべき乗サイズを保つように注意しましょう。2
のべき乗サイズでない場合、浮動小数点（float 型）として処理がしづらく、パフォーマンス
が落ちたり画像の縮小がうまくできなくなってしまいます。
そのため、これらを一つのファイルにまとめることができる仕組みがあります。この単一ファ
イルはglb（glTF-Binary）と呼ばれています。
gltf Converter全部一つにする
「glTF ビューアー（ https://gltf-viewer.donmccurdy.com/ ）
GLTFLoader はglTF のDraco 圧縮拡張にも対応
アニメーションは一つしか出せない　いまのところ

頂点情報はどうする？
普通に入ってる　gltf.scene

アニメーションは複数やる場合自力で追加すればいいじゃない
できそう。gltfじゃないほうがいいかも
最初の女の子はブレンダーエクスポーターだからできた

draco圧縮レンダリングせいどは同じ
展開時が一番重い
でかいファイルのみdraco圧縮しよう

### 0624
深度テスト
深度バッファとカラーバッファは同じ解像度
深度テストは、WebGL の API としては gl.enable メソッドを使うことで切り替えできます。
```
// enable メソッドで引数の内容に応じてフラグが有効に設定される
// gl.DEPTH_TEST で深度テストを有効化できる
gl.enable(gl.DEPTH_TEST);
```
記述のタイミングはドローコールの前ならどこでもいい
行列で座標変換しないとダメ

カリング
深度テストを有効にすると裏面は描画されない
裏表の判定は頂点の結び方

カメラから見て反時計回りだと表
負荷が軽減できるから有効化しておいたほうがいい
```
// カリングを有効化する
gl.enable(gl.CULL_FACE);

// BACK 裏面をけす
// FRONT　表面をけす
gl.cullFace(gl.FRONT);     // カリング面の設定
```

行列
http://school.souhonzan.org/201804/004p/
ベクトルを「あるルールに添って」掛けて足してるだけ
3D 数学の座標変換に出てくる行列も基本的にこれと同じ！
ベクトル（x, y, z）を変換する行列は、列数が３になっていればいい
もっと簡潔に言うなら、行列とは単に ベクトルを受け取ってベクトルを返す関数 みたいなもの！

行列の特性
行列は座標変換を行なってくれる関数のようなもの
様々な効果を持ついろいろな行列（関数）がある
行列は行列同士で掛け合わせることができ、このとき複数の効果が合成される
行列が活躍するのは頂点の座標変換のとき、つまり頂点シェーダのフェーズ
最終的にシェーダに送る行列はたったひとつに集約させることができる

座標変換のための三大行列
モデル座標変換行列
・平行移動、回転、拡大縮小！

ビュー座標変換行列
・ひとことで言えばカメラ

プロジェクション座標変換行列
・どこからどこまでをスクリーンに投影するのか

9番は課題用

照明をつくる
法線によって色の付き方がかわる
法線は、向きだけを表すために用いられるベクトルです。
頂点に座標、色、に続き、法線という頂点属性を持たせる必要があるわけです。

ライトベクトル
ベクトルだけど向きだけuniformで渡す　単位化する
dot は、ベクトルの内積
ベクトルの内積とは
ベクトルの掛け算というのは厳密には存在しなくて、その代わり 内積や外積 といった計算方法が使えるのでした。

内積の利用方法
戻り値が ベクトルではなく数値としてでる
ベクトル同士のシンクロ率
正規化したベクトル同士の内積は ベクトルの向きのシンクロ率 とみなせる
```
attribute vec3 position; //頂点位置
attribute vec3 normal; //頂点の法線
attribute vec4 color; //頂点カラー
uniform mat4 mvpMatrix;
varying vec4 vColor;

// 平行光源（ディレクショナルライト）のライトベクトル @@@
// これはuniformで渡してもいい
// 向きだけに注目したいから正規化
const vec3 light = normalize(vec3(1.0));

void main(){
    // ベクトルの内積を用いて照度を計算する @@@
    // max 大きい方を返す
    // dot　内積
    // ライトベクトも法線も単位ベクトルである（長さ１）
    // dotというのはベクトルの内積を計算にするもの
    // 内積の結果は２つのベクトルのシンクロ率
    // 単位ベクトル同士の内積の結果は　-1.0 ~ 1.0になる
    // シンクロ率が低いほど暗い色になる
    float diffuse = max(dot(normal, light), 0.0);
    // 求めた照度を頂点の色に乗算する @@@
    vColor = color * vec4(vec3(diffuse), 1.0);
    gl_Position = mvpMatrix * vec4(position, 1.0);
}
```
```
内積の計算方法
内積の計算方法は、こうです。

V = [x1, y1, z1];
W = [x2, y2, z2];
V・W === (x1 * x2) + (y1 * y2) + (z1, z2);
```

回転をすると光が反映されてない
動きに合わせて再計算していないから
法線も更新する
行列で計算する
法線変換用の行列を作る
法線変換用の行列は、モデル座標変換行列の逆転置行列 を使います。
```
法線を正しい向きにする変換行列
// ※ mMatrix == モデル座標変換行列
// ※ normalMatrix == 法線変換用行列を格納する変数

// モデル座標変換行列を転置
mat4.transpose(mMatrix, normalMatrix);

// さらに行列を逆行列に変換する
mat4.inverse(normalMatrix, normalMatrix);
```

頂点単位で照明効果を計算する手法のことを、グーローシェーディングと呼んでいます。
これをフラグメントシェーダ側で陰影計算するように変更する
より精細なライティングになるが、反面、負荷は若干高くなる
頂点単位なのか、ピクセル単位なのかの違い

サイト制作で役立つthreejsAPI
株式会社つみき
小林陽一

デイレクターデザイナーが３dグラフィックを理解しない
とりあえずCGやりたいから開発してみたいな炎上がおきる
webglのメリットを誰も説明できない
クライアントが求めていなならやらない

センスがあるやつ無理
デザインもコードもできるやつが作るやーつだから

サイバースペースなどイメージしやすい
攻殻機動隊みたいなやつ

buffergeometry　ジオメトリを１から定義
rawshadermaterial　独自シェーダー

最初からジオメトリを作るやつもあるよね

shadermaterial 基本的な属性を定義してくれる
bufferAttribute buffergeometryに属性を定義する

サイバースペースだと
全部半透明
加算合成
深度なし
ライティングなし
無駄なものを置かない
処理を軽くしたい（独自シェーダーやる）

instancedBuffterGeomtry
一つのオブジェクトを大量に生成

instancedBuffterAtribute
いんたんしんぐで大量において物量でなんとかする

three.point
パーティクル
サイバースペースはパーティクルだらけ

工夫をこらす
小手先の工夫
UVをいじる
テクスチャの座標をいじる
UVアニメーションでビームを飛ばす

canvas2Dをつかう
パーティクルの頂点情報の生成に利用する

canvas2dで色情報もらえる

domを使う
字はdom
objet#dえおcameraのぎょうれつ
ｊｓでスクリーン座標変換して得た値を
trasnformで動かす

第一線で活躍するにはもっと基礎的なことを固めないとだめ
制作現場で生きていくもの

いんすたんしんぐとは
drow arrys  = バインドされてるVBOを描画する

いんすたんしんぐとは
vboを足して一気に描画する
軽くする
一個一個制御するのが面倒
ちゃんと制御しましょう

# 0707
テクスチャをはる
画像を読みこんだり動画ファイルの映像を読みこんだりしながら、これをポリゴンに貼り付ける（マッピング）することができます。このような処理のことを テクスチャマッピング と呼ぶ
WebGL では、拡張機能などを使わずに安全に読み込める画像形式は JPEG と PNG の二者です
テクスチャの元データとなる画像は、画像の一辺のピクセル数（サイズ）を 2 の累乗にあわせる必要がある（縦横の比率は１：１でなくても大丈夫！）
画像データの大きさの上限値も実は存在するが端末によって違う
https://alteredqualia.com/tmp/webgl-maxparams-test/
https://webglstats.com/
スマホは4096px

テクスチャユニット
テクスチャが最大で何枚利用できるのかは決まってる
テクスチャユニットとはGPUになんこ画像を置けるか
最低でも８つは存在する

```
テクスチャユニットまとめ
テクスチャユニットはテクスチャが格納される場所
テクスチャの最大利用数 === テクスチャユニット数
アクティブなユニットに対してバインドされる
ユニットを切り替えてバインドすれば同時に複数のテクスチャをバインドすることも可能
```
頂点属性にてテクスチャの座標を追加する
OpenGL や GLSL では、S と T を使ってこのテクスチャ座標
GLSL の内部でテクスチャの色情報を抜き出す際には　texture2D

テクスチャパラメータ
テクスチャに対して設定するパラメータのこと
・テクスチャ参照時の補間方法
filter設定　テクスチャのドットを保管
```
ニアレストネイバー　色を混ぜない　初代ps
gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
```

ミップマップ
ミップマップとは、元のイメージデータの半分の大きさのテクスチャをあらかじめ生成しておき、内部的に持っておく仕組みです。
1pxまで小さくする


範囲外テクスチャ座標
```
gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, 設定値);
gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, 設定値);

// 設定値
gl.REPEAT // 繰り返し
gl.MIRRORED_REPEAT // 反転繰り返し
gl.CLAMP_TO_EDGE // 切り捨て
```

テクスチャパラメータまとめ
バインドしているテクスチャにパラメータを設定できる
フィルタリングの方法
ミップマップは縮小フィルタにしか使えない
範囲外のテクスチャ座標が指定された場合の挙動


ブレンディング
色と色、この両者が表すものは厳密には SRC と DST というふたつのキーワードで表される色
SRC は、今から出力しようとしている色
DST は、出力済みの色 です
フラグメントシェーダよりもさらに先のステージで色を操作
ブレンドは有効にしただけで重くなる可能性があるということ
ブレンディングは GLSL とは直接関係のない部分で処理する
```
ブレンディングの有効化
有効化には gl.enable メソッドを利用します。

引数には gl.BLEND を与えます。

// 記述例
gl.enable(gl.BLEND);

gl.blendFuncSeparate(
  gl.SRC_ALPHA,           // SRC_RGBを使う
  gl.ONE_MINUS_SRC_ALPHA, // DST_RGB使う
  gl.ONE,                 // SRC_A
  gl.ONE                  // DST_A
);
```
https://wgld.org/s/sample_018/

画像処理の知識がないと難しい
裏面から見ないと透けないぞい（震度テストによって消えてるから）
深度テストとブレンドは相性が悪い
深度テストを無効化すると重なりが聞かなくなる
レーザービームは加算合成

ブレンドとテクスチャは設定が多いから慣れよう

ただし注意点も
if分はあまりつかわないほうがいい
条件分岐の負荷は昔はすごく高い
多重if分はやめたほうがいい

カリングを利用したエッジ描画
エッジライン用のモデルを 表をカリングした状態で レンダリングする。

フォグ　
距離フォグ
距離フォグ とか 深度ベースのフォグ
カメラからの距離によって変化する

インスタンシング
WebGL 2.0なら標準で使える
WebGL 1.0 はインスタンシングを標準ではサポートしていない
WebGL の拡張機能
ジオメトリ・インスタンシングは、WebGL 1.0 では拡張機能を有効化することで利用することができる
https://www.khronos.org/registry/webgl/extensions/

```
// インスタンシングのための拡張機能を有効化する
ext = gl.getExtension('ANGLE_instanced_arrays');
if(ext == null){
    alert('ANGLE_instanced_arrays not supported');
    return;
}
```

インスタンシングを利用した描画では、なんと描画命令は たった１回しか発行しない にもかかわらず
複数の（大量の）オブジェクトを描画 できます。
同じ形状なら大量に生成できる

紫くん講演会
windows機だった
webStome使い
サーファイスブック使い
npm派
typescript勢
webpack使い

クリエイティブコーディング
ビジュアルアーティスト
モーショングラファー

時間が短いとチャレンジしにくい（それな）
シミュレーションする
３DCGをつくる

DUPで送られてこない
midiでこない

modi
ポストエフェクトかけないほうがいい

勉強したあとなにするか
作りたいものがなにかわからない
クリエイティブコーディングという分野
processeing
ダニエルシフマン最高
ネイチャーコード本が最高
p5jsのジェネレーティブデザイン本がいい
商業主義から離れた活動
load lineすごい
北千住デザイン独立したらしい
中村勇吾もそう
ジョン前田　ポストスクリプト時代
古の言語
プログラムからデザインを作る

クリエイティブコーデイングとは
oFザック曰く
詩の本とコーディングを組み合わせる
他のコンテンツから持ってくる
いろんな媒体の間にプログラムをいれる

ライゾマ　oFめっちゃ使ってる
C++ for文１０００回やってもパフォーマンスですからすごい

AAA級ゲーム　映画VFX　
最近のゲームレンダリング計算はほとんどPBR
フォトリアル
紫くん最近学生やってる　MAYAでCGやってる　たぶんデジハリ
小さいものをこつこつ作ることに楽しむを見出すこと
30フレ　６０フレを出すことを目的とするべきではない

デッサンする
よくみると一本一本の線があらい
ここは細かく書いたほうがいいところと荒く書いていい場所を判断できる
そうするとディテールを積めれるようになる
細かく観察する

ブラクラ表現
パーリンノイズで動かして生きてるかんをだす
可愛く見えてくる
guiのパラメーターめっちゃある
OSのUI一覧サイトがある

表現のストックをする

インスタンシングの物量で’埋めるのではなく
how to play fill spector
使いづらいUIとかっていう考えもあり

plug play

プライベートで
ゲームっぽいことをしようとするとしんどい
矩形並べたりとかそういうことしたほうがいい

oF
Unity
WebGL
の反復横飛び中


# 0721
## 点として頂点を描く
GLSL には、gl_PointSize という組み込み変数があります。
ポイントサイズの指定は頂点シェーダ内でのみ行うことができます。
点描画で描かれるのは矩形

頂点シェーダでテクスチャを使う（VTF
VTF は Vertex Texture Fetch

## ポイントスプライト
点で描くときにそれが矩形になってしまうというのは、ちょっと格好悪く感じてしまう場合もあるかもしれません。
頂点を点で描きつつ、そこにテクスチャを貼り付けて見た目をリッチにする
桜とか雪とかにむいてる
もし板ポリゴンではなく点で描かれているとしたら、これまでのように、四角形のポリゴンを構成する頂点がテクスチャ座標を持っているわけではありませんよね。ではいったいどのようにしてテクスチャ座標を求めているのでしょうか。
これには、GLSL の組み込み変数 gl_PointCoord を使います。

あくまでも点なのでパースなどは掛からない点に注意

## 環境マッピング
キューブ環境マッピング
たとえば、鏡とか、ガラスの表面とか、金属の表面といったような、周辺の 環境の様子がマッピングされる 質感を実現するために用いられます。
ちょうど、正立方体のカタチをした部屋の中にすっぽりと入ってしまい、そこから景色を眺めるような、そんな状態を再現します
六面体の各面に、風景に相当するテクスチャを設定します。
この gl.TEXTURE_CUBE を指定されたテクスチャには、当たり前ですが 正六面体すべて の面を構成する画素を割当てしてやる必要があります
テクスチャここにいっぱいある
http://www.humus.name/index.php?page=Textures
CGはまやかし

## 屈折マッピング
屈折ベクトルの計算
```
// 頂点座標とカメラの位置から視線ベクトルを算出（正規化する）
vec3 eyeDirection = normalize(vPosition - eyePosition);
refractVector = refract(eyeDirection, n, eta);
```
reflect は反射、refract は屈折！

屈折データ
http://ww1.tiki.ne.jp/~uri-works/tmp/

現実世界の屈折とまったく同じ状態を完全に再現できるわけではありません。
あくまでも屈折ベクトルを使ってそれっぽく近似しながらテクスチャを参照しているだけです。
こういった写り込みや反射、屈折といった処理は、実はレイトレーシングなどの技術を用いたほうがスマートに実装できる場合が多いです。
レイトレーシングは一晩かけて一枚の画像作りましたの世界
げんざいではリアルタイムでもいける


## フレームバッファ
オフスクリーンレンダリング
一回のレンダリングでやってた
フレームバッファを用いると、この１パスでのレンダリング、という壁を超えた高い表現が可能になります。
オフスクリーンレンダリングとは、画面上には映らないけれども、内部的な描画領域を作って、そこに対して描画を行うことを言います。
TD　オフスクリーンレンダリングのかたまり
一度メモリ上に一時的な描画を行い、それを参照しながら さらに手を加えたり、複数のシーンを合成したりする
目に得ないバッファ　深度バッファ
フレームバッファのなかにレンダーバッファをいれ、レンダーバッファにフォーマットを指定する
フォーマットはたくさんある
gl.RGBA4、gl.DEPTH_COMPONENT16、など
gl.RGBA4の４はビット
gl.DEPTH_COMPONENT16の１６はビット
4 ビットでは、16 段階しかデータを細分化できません
８ビットなら255
一般にカラーバッファには テクスチャオブジェクトを割り当てて使う 方法を用います。
```
フレームバッファはバッファの入れ物
レンダーバッファをフレームバッファに格納する
レンダーバッファにはフォーマットを指定する
カラーバッファ用にはレンダーバッファよりもテクスチャを使ったほうがよい
```
カメラで撮影してフォトショで加工するみたいな流れ

## emadurandalさん
力が欲しいか？では武器を持て
今の時代、Unityで全部できちゃうのに？
・Unityなどのツールより、よりきめ細かい制御が自由にできます・勉強になり、知見が広がります（CG理論、プログラムスキル向上）
ゲームで例えるなら、「（魔術の仕組みを知っている）魔法使いが唱える魔法」...ではなく、「魔法を中に詰めた魔法アイテム」を一般市民が使うようなもの。
現代技術という名の魔法アイテムは便利だが、
・あらかじめ作られた範囲しか、その機能を引き出せない。
・故障や誤動作がおきたら、お手上げ
・組み合わせることができない（出来たとしても、バリエーションが限られる）
・（なによりも）仕組みがわからないので、悪用された場合やられっぱなしに！
https://viewer.glboost.org/
https://effekseer.github.io/jp/
GLBoostだけで飯が食える

空間情報が違うと計算がおかしくなるので、ワールド空間に合わせてる
threejsは25本しかボーンはいけない
